...
gold_result_merged = ([('https://clarin09.ims.uni-stuttgart.de/',
                        'https://clarin09.ims.uni-stuttgart.de/', {'tag': 'a'}),
                       ('https://clarin09.ims.uni-stuttgart.de/sdc_warc/index.html',
                        'https://clarin09.ims.uni-stuttgart.de/sdc_warc/page1.html',
                        {'tag': 'a'}),
                       ('https://clarin09.ims.uni-stuttgart.de/sdc_warc/index.html',
...
class TestLinkextractionMethods(unittest.TestCase):
    ...
    def test_wmd(self):
        with open(warc_path, "rb") as warc_file:
            ...
            for i, record in enumerate(ai):
                if i == metadata_index:
                    self.assertCountEqual(linkextraction._wmd(record, blacklist=[]), gold_result_index)
                    break

    def test_bs4(self):
        self.assertCountEqual(linkextraction._bs4(this_html, this_url, blacklist=[]), gold_result_index)

    def test_rep(self):
        browser = linkextraction._set_up_browser()
        self.assertCountEqual(linkextraction._rep(this_html, this_url, blacklist=[], browser=browser),
                              gold_result_index)
    ...
    def test_get_links_from_warc(self):
        ...
        links, node_attribs = linkextraction._get_links_form_warc(warc_path, methods=methods, include_content=False,
                                                                  count_tags=False, blacklist=[], browser=browser)
    ...
    def test_get_links_from_live(self):
        ...
        links, node_attribs = linkextraction._get_links_from_live(urls, methods, blacklist=[], include_content=False,
                                                                  count_tags=False, browser=browser)
        self.assertEqual(type(links), dict)
        self.assertEqual(node_attribs, {})
    ...
    def test_extract_links(self):
        links, node_attribs = linkextraction.extract_links(warc_path)
